...
ingest:         ## 법제처 법령 수집 + 임베딩 (LAW_API_KEY, OPENAI_API_KEY 필수)
	uv run python -m app.ingestion.runner

ingest-collect: ## 법령 수집 + DB 저장만 (임베딩 생략, LAW_API_KEY 필수)
	uv run python -m app.ingestion.runner --skip-embed

ingest-dry:     ## 수집 파싱 테스트 (DB 반영 없음, LAW_API_KEY 필수)
	uv run python -m app.ingestion.runner --dry-run

ingest-debug:   ## API 요청/응답 원문 출력 (dry-run 포함, LAW_API_KEY 필수)
	uv run python -m app.ingestion.runner --dry-run --debug
...
format:         ## 코드 자동 포맷 (ruff format + isort)
